...
V2.6.10 — BOOT / IMPROV READY
- Base: V2.6.7.
- Rev.11 e todos os BINs inalterados.
- sens-improv.js inalterado.
- Mudança única: preparação da serial ANTES de Nome/Serial.
- Após abrir a porta: 2,6 s de estabilização + até 4 handshakes Improv,
  espaçados em 1,4 s + guarda final de 0,5 s.
- Fluxo de Nome, Serial, scan e Wi-Fi permanece o da base.

V2.6.10 — AUTO RECOVERY
- Se o primeiro handshake Improv não responder, reabre automaticamente a mesma porta USB.
- O usuário não precisa repetir o clique da Etapa 3.
- O scan Wi-Fi executa um segundo ciclo completo automático antes do fallback manual.
- Firmware Rev.11 e binários permanecem inalterados.

V2.6.10 — PAINEL DE CONFIGURAÇÃO + MONITOR SENS
- Base funcional: V2.6.9 AUTO RECOVERY.
- Firmware Rev.11 e todos os BINs inalterados.
- Novo painel visível com a configuração da imagem instalada:
  firmware, hardware, SDF, intervalo, papel, identidade, Wi-Fi e sensores.
- Senha Wi-Fi nunca é exibida.
- ETAPA 5 — MONITOR SENS:
  liberado somente após SENS OPERACIONAL;
  console Web Serial 115200 bps diretamente na página;
  Abrir / Desconectar / Limpar;
  tentativa de manter DTR/RTS inativos para reduzir resets ao abrir a serial.
- Nenhuma alteração na SENS Platform, Epoch, Seq, aquisição ou protocolo científico.
