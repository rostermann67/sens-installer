...
export class SensImprov {
  constructor(port, log=()=>{}) {
    ...
    this.rawMode=false; this.rawBuf=""; this.rawWaiter=null;
  }
  async open() {
      ...
      while(this.running){
        ...
        for(const b of value){
          if(this.rawMode){
            ...
            if(ch==='\r') continue;
            if(ch==='\n'){
              const line=this.rawBuf; this.rawBuf="";
              if(line.startsWith("SENS15C|")) this._rawOnLine(line);
              continue;
            }
            if(b>=32 && b<=126){
              if(this.rawBuf.length<600) this.rawBuf+=ch; else this.rawBuf="";
            } else this.rawBuf="";
            continue;
          }
  ...
  // Rev.15C — troca de protocolo SEM fechar/reabrir a SerialPort.
  // O mesmo reader/writer que confirmou a identidade passa a enquadrar linhas ASCII.
  enterSens15C(){
    if(this.pending) throw new Error("Ainda há comando Improv pendente.");
    this.buf=[]; this.rawBuf=""; this.rawWaiter=null; this.rawMode=true;
  }
  leaveSens15C(){ this.rawMode=false; this.rawBuf=""; this.rawWaiter=null; this.buf=[]; }
  _rawOnLine(line){
    if(!this.rawWaiter) return;
    try{this.rawWaiter.onLine(line);}catch(e){const w=this.rawWaiter;this.rawWaiter=null;w.reject(e);}
  }
  async _rawWrite(line){ await this.writer.write(new TextEncoder().encode(line+"\n")); }
  async _rawExpect(line,accept,timeout=7000){
    if(this.rawWaiter) throw new Error("Comando serial Rev.15C anterior ainda em andamento.");
    return await new Promise(async(resolve,reject)=>{
      const timer=setTimeout(()=>{this.rawWaiter=null;reject(new Error("Tempo esgotado aguardando ACK serial Rev.15C."));},timeout);
      this.rawWaiter={reject,onLine:(row)=>{
        if(row.startsWith("SENS15C|ERR|")){clearTimeout(timer);this.rawWaiter=null;reject(new Error(row));return;}
        const v=accept(row); if(v!==undefined){clearTimeout(timer);this.rawWaiter=null;resolve(v);}
      }};
      try{await this._rawWrite(line);}catch(e){clearTimeout(timer);this.rawWaiter=null;reject(e);}
    });
  }
  async sens15cBegin(token){
    return await this._rawExpect(`@SENS15C|BEGIN|${token}`,r=>r===`SENS15C|ACK|BEGIN|${token}|READY`?true:undefined);
  }
  _sens15cChunks(text,max=72){
    const out=[]; let cur=""; const enc=new TextEncoder();
    for(const ch of String(text??"")){const next=cur+ch;if(cur&&enc.encode(next).length>max){out.push(cur);cur=ch}else cur=next;}
    if(cur||!out.length)out.push(cur); return out;
  }
  async sens15cSection(tag,text){
    const chunks=this._sens15cChunks(text,72);
    for(let i=0;i<chunks.length;i++){const last=i===chunks.length-1?1:0;
      await this._rawExpect(`@SENS15C|${tag}|${i}|${last}|${chunks[i]}`,r=>r===`SENS15C|ACK|${tag}|${i}|${last}`?true:undefined);
    }
  }
  async sens15cCommit(){
    if(this.rawWaiter) throw new Error("Comando serial Rev.15C anterior ainda em andamento.");
    return await new Promise(async(resolve,reject)=>{
      const rows=[]; const timer=setTimeout(()=>{this.rawWaiter=null;reject(new Error("Tempo esgotado no COMMIT Rev.15C."));},26000);
      this.rawWaiter={reject,onLine:(row)=>{
        if(row.startsWith("SENS15C|ERR|")){clearTimeout(timer);this.rawWaiter=null;reject(new Error(row));return;}
        rows.push(row); if(row==="SENS15C|END"){clearTimeout(timer);this.rawWaiter=null;resolve(rows);}
      }};
      try{await this._rawWrite("@SENS15C|COMMIT");}catch(e){clearTimeout(timer);this.rawWaiter=null;reject(e);}
    });
  }
