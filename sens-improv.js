...
const CMD_SENS_GET_ADV=0x22, CMD_SENS_SET_ADV=0x23, CMD_SENS_GET_GENERIC=0x24, CMD_SENS_SET_GENERIC=0x25, CMD_SENS_GET_CORR=0x26, CMD_SENS_SET_CORR=0x27;
...
export class SensImprov {
  ...
  async setDeviceName(name) {
    return (await this.rpc(CMD_DEVICE_NAME,[...new TextEncoder().encode(name)],15000))[0]||"";
  }
  async setHostname(host, timeout=12000) {
    return (await this.rpc(CMD_HOSTNAME,[...new TextEncoder().encode(host)],timeout))[0]||"";
  }
  async getDeviceName(){return (await this.rpc(CMD_DEVICE_NAME,[],12000))[0]||"";}
  async getHostname(){return (await this.rpc(CMD_HOSTNAME,[],12000))[0]||"";}
