...
export class SensImprov {
  ...
  async setHostname(host, timeout=5000) {
  ...
  async resyncChannel(waitMs=900) {
    this.pending=null;
  ...
  async setAndConfirm(setter, getter, wanted, label, onAttempt=()=>{}) {
    let lastError=null;
    for(let i=1;i<=3;i++){
      onAttempt(i,3);
      try{
        ...
        try{
          ...
          if(readback===wanted) return readback;
          if(returned===wanted) return returned;
          lastError=new Error(`${label} retornado não confere: "${readback}"`);
        }catch(e){
          if(returned===wanted) return returned;
          lastError=e;
        }
      ...
      await this.resyncChannel(900);
    }
    throw lastError || new Error(`${label} não foi confirmado.`);
  }

  async provisionReliable(ssid,password,onAttempt=()=>{}) {
    let lastError=null;
    for(let i=1;i<=3;i++){
      onAttempt(i,3);
      try{
        const result=await this.provision(ssid,password);
        return result;
      }catch(e){
        lastError=e;
        // Se o ESP ainda estiver terminando uma tentativa, espere antes de repetir.
        await new Promise(r=>setTimeout(r,1800));
        await this.resyncChannel(700);
      }
    }
    throw lastError || new Error("Não foi possível provisionar o Wi-Fi.");
  }
  ...
  async scanReliable(onAttempt=()=>{}) {
    ...
    await new Promise(r=>setTimeout(r,1800));
    let lastError=null;
    const timeouts=[15000,25000,30000];
