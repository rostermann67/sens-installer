...
export class SensImprov {
  ...
  async sensFullBasic(encoded){
    return await this.rpc(CMD_SENS_FULL_BASIC,[...new TextEncoder().encode(encoded)],14000);
  }
  async sensFullAdvanced(encoded){
    return await this.rpc(CMD_SENS_FULL_ADV,[...new TextEncoder().encode(encoded)],14000);
  }
  async sensFullGeneric(idx,encoded){
    return await this.rpc(CMD_SENS_FULL_GENERIC,[...new TextEncoder().encode(String(idx)+"|"+encoded)],14000);
  }
  async sensFullCorrection(idx,encoded){
    return await this.rpc(CMD_SENS_FULL_CORR,[...new TextEncoder().encode(String(idx)+"|"+encoded)],14000);
  }
  async sensFullCommit(){
    return await this.rpc(CMD_SENS_FULL_COMMIT,[],24000);
  }
